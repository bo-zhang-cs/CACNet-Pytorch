...
def evaluate_on_FCDB_and_FLMS(model, dataset, save_results=False):
    ...
    with torch.no_grad():
        ...
        for dataset in test_set:
            test_dataset = dataset(
                split="test", keep_aspect_ratio=cfg.keep_aspect_ratio
            )
            ...
            for batch_idx, batch_data in enumerate(tqdm(test_loader)):
                ...
                if save_results:
                    ...
                    croped_img = source_img[
                        best_crop[1] : best_crop[3], best_crop[0] : best_crop[2]
                    ]
    ...
    print(
        "Test on {} images, IoU={:.4f}, Disp={:.4f}, recall={:.4f}(iou>={:.2f})".format(
...
def evaluate_composition_classification(model):
    ...
    print(f"Test on {total} images, {correct} Correct, Acc {acc:.2%}")
    for i in range(len(cls_cnt)):
        print(
            "{}: total {} images, {} correct, Acc {:.2%}".format(
                composition_cls[i],
                cls_cnt[i],
                cls_correct[i],
                float(cls_correct[i]) / cls_cnt[i],
            )
        )
    return acc
