...
class CompositionModel(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(256, 128, kernel_size=1, padding=0), nn.ReLU(True)
        )
        self.conv4 = nn.Sequential(
            nn.Conv2d(128, 128, kernel_size=1, padding=0), nn.ReLU(True)
        )
    ...
    def forward(self, f2, f3, f4):
        ...
        with torch.no_grad():
            ...
            kcm = F.interpolate(
                kcm, scale_factor=4, mode="bilinear", align_corners=True
            )
            return logits, kcm
...
def generate_anchors(anchor_stride):
    assert anchor_stride <= 16, f"not implement for anchor_stride{anchor_stride} > 16"
...
class PostProcess(nn.Module):
    ...
    def forward(self, offsets, kcm):
        ...
        offsets = einops.rearrange(
            offsets, "b (n c) h w -> b n h w c", n=self.num_anchors, c=4
        )
        coords = [
            F.pixel_shuffle(offsets[..., i], upscale_factor=self.upscale_factor)
            for i in range(4)
        ]
