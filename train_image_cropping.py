...
import time
import warnings
from test import evaluate_composition_classification, evaluate_on_FCDB_and_FLMS
...
def create_dataloader():
    ...
    print(
        "FCDB training set has {} samples, batch_size={}, total {} batches".format(
            len(crop_dataset), cfg.crop_batch_size, len(crop_loader)
        )
    )
    com_dataset = CompositionDataset(
        split="train", keep_aspect_ratio=cfg.keep_aspect_ratio
    )
    ...
    print(
        "Composition training set has {} samples, batch_size={}, total {} batches".format(
...
class Trainer:
    ...
    def visualize_com_prediction(self, image_path, logits, kcm, category):
        ...
        gt_ss = f"gt:{gt_label}"
        dst = cv2.putText(
            dst, gt_ss, (20, 40), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2
        )
        pr_ss = f"pre:{label}"
        dst = cv2.putText(
            dst, pr_ss, (20, 80), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2
        )
        # h,w,1
        ...
        cv2.imwrite(os.path.join(self.visual_path, f"com-{self.iters}.jpg"), fuse_im)
    ...
    def train(self):
        ...
        for batch_idx, batch_data in enumerate(self.crop_loader):
            ...
            if batch_idx > 0 and batch_idx % cfg.display_freq == 0:
                ...
                time_per_batch = (time.time() - start) / (batch_idx + 1.0)
                last_batches = (self.max_epoch - self.epoch - 1) * total_batch + (
                ...
                print(
                    "=== epoch:{}/{}, step:{}/{} | Crop_Loss:{:.4f} | Com_Loss:{:.4f} | Com_acc:{:.2%} | lr:{:.6f} | estimated last time:{} ===".format(
                        self.epoch,
                        self.max_epoch,
                        batch_idx,
                        total_batch,
    ...
    def eval(self):
        ...
        for m in self.best_results.keys():
            ...
            if update:
                ...
                checkpoint_path = os.path.join(cfg.checkpoint_dir, f"best-{m}.pth")
                torch.save(self.model.state_dict(), checkpoint_path)
                print(f"Update best {m} model, best {m}={self.best_results[m]:.4f}")
            if m in ["FCDB_iou", "FLMS_iou"]:
                self.writer.add_scalar(f"test/{m}", epoch_result[m], self.epoch)
                self.writer.add_scalar(
                    f"test/best-{m}", self.best_results[m], self.epoch
                )

        if self.epoch > 0 and self.epoch % cfg.save_freq == 0:
            checkpoint_path = os.path.join(
                cfg.checkpoint_dir, f"epoch-{self.epoch}.pth"
            )
            torch.save(self.model.state_dict(), checkpoint_path)
