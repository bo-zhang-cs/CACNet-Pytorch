...
class CompositionDataset(Dataset):
        ...
        with open(self.label_txt) as f:
            for line in f.readlines():
                ...
                if txt_idx < int(os.path.splitext(image_name)[0]):
                    continue
                image_idx += 1
                assert int(os.path.splitext(image_name)[0]) == txt_idx, [
                    txt_idx,
                    image_name,
                    line,
                ]
            ...
            print(f"{self.split} set, total {len(annotation)} images")
        return annotation
    ...
    def gather_training_annotation(self):
        ...
        for c in range(9):
            print(f"{composition_cls[c]}, total {len(annotation[c])} training images")
        return annotation
